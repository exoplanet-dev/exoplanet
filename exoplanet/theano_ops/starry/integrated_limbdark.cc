...
int APPLY_SPECIFIC(integrated_limbdark)(
  ...
  auto f = allocate_output<DTYPE_OUTPUT_0>(ndim, dims, TYPENUM_OUTPUT_0,
                                           output0, &success);
  auto dfdcl = allocate_output<DTYPE_OUTPUT_1>(
      ndim + 1, &(shape[0]), TYPENUM_OUTPUT_1, output1, &success);
  auto dfdr = allocate_output<DTYPE_OUTPUT_2>(ndim, dims, TYPENUM_OUTPUT_2,
                                              output2, &success);
  auto dfdx = allocate_output<DTYPE_OUTPUT_3>(ndim, dims, TYPENUM_OUTPUT_3,
                                              output3, &success);
  auto dfdxt = allocate_output<DTYPE_OUTPUT_4>(ndim, dims, TYPENUM_OUTPUT_4,
                                               output4, &success);
  auto dfdxtt = allocate_output<DTYPE_OUTPUT_5>(ndim, dims, TYPENUM_OUTPUT_5,
                                                output5, &success);
  auto dfdy = allocate_output<DTYPE_OUTPUT_6>(ndim, dims, TYPENUM_OUTPUT_6,
                                              output6, &success);
  auto dfdyt = allocate_output<DTYPE_OUTPUT_7>(ndim, dims, TYPENUM_OUTPUT_7,
                                               output7, &success);
  auto dfdytt = allocate_output<DTYPE_OUTPUT_8>(ndim, dims, TYPENUM_OUTPUT_8,
                                                output8, &success);
  npy_intp empty[] = {};
  auto num_eval = allocate_output<DTYPE_OUTPUT_9>(0, empty, TYPENUM_OUTPUT_9,
                                                  output9, &success);
  if (success) return 1;

  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_1, Eigen::Dynamic, Eigen::Dynamic,
                           Eigen::RowMajor>>
      dfdcl_mat(dfdcl, Nc, Nr);
  dfdcl_mat.setZero();
  ...
  auto func =
      vice::functors::LimbDarkFunctor<Diff, Scalar>(APPLY_SPECIFIC(L), cvec);
  ...
  for (npy_intp i = 0; i < Nr; ++i) {
    ...
    if (z[i] > 0) {
      ...
      int nlim = func.setup(r_, x_, xt_, xtt_, y_, yt_, ytt_, z[i], zt[i],
                            dt[i], limits, include_contacts);
      auto ym = func(limits[0]);
      for (int j = 0; j < nlim - 1; ++j) {
        auto yp = func(limits[j + 1]);
        auto val = integrator(func, limits[j], limits[j + 1], ym, yp, tol,
                              max_depth, min_depth) /
                   dt[i];
        f[i] += val.value();
