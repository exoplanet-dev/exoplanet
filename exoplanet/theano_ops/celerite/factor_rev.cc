    ...
    PyArrayObject** output3)
{
  ...
  auto ba_out    = allocate_output<DTYPE_OUTPUT_0>(1, shape0, TYPENUM_OUTPUT_0, output0, &success);
  auto bU_out    = allocate_output<DTYPE_OUTPUT_1>(2, shape1, TYPENUM_OUTPUT_1, output1, &success);
  auto bV_out    = allocate_output<DTYPE_OUTPUT_2>(2, shape2, TYPENUM_OUTPUT_2, output2, &success);
  auto bP_out    = allocate_output<DTYPE_OUTPUT_3>(2, shape3, TYPENUM_OUTPUT_3, output3, &success);
  if (success) return 1;
  ...
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_4, Eigen::Dynamic, CELERITE_J2, CELERITE_J_ORDER>> S(S_in, N, J*J);
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_5, Eigen::Dynamic, 1>>                             bd(bd_in, N);
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_6, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>>  bW(bW_in, N, J);

  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_0, Eigen::Dynamic, 1>>                            ba(ba_out, N);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_1, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> bU(bU_out, N, J);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_2, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> bV(bV_out, N, J);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_3, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> bP(bP_out, N-1, J);
