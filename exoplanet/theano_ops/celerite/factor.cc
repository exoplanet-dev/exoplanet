    ...
    PyArrayObject** output3)
{
  ...
  npy_intp shape3[] = {};
  auto d_out    = allocate_output<DTYPE_OUTPUT_0>(1, shape0, TYPENUM_OUTPUT_0, output0, &success);
  auto W_out    = allocate_output<DTYPE_OUTPUT_1>(2, shape1, TYPENUM_OUTPUT_1, output1, &success);
  auto S_out    = allocate_output<DTYPE_OUTPUT_2>(2, shape2, TYPENUM_OUTPUT_2, output2, &success);
  auto flag_out = allocate_output<DTYPE_OUTPUT_3>(0, shape3, TYPENUM_OUTPUT_3, output3, &success);
  if (success) return 1;

  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_0, Eigen::Dynamic, 1>>                            a(a_in, N);
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_1, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> U(U_in, N, J);
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_2, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> V(V_in, N, J);
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_3, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>> P(P_in, N-1, J);

  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_0, Eigen::Dynamic, 1>>                             d(d_out, N);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_1, Eigen::Dynamic, CELERITE_J, CELERITE_J_ORDER>>  W(W_out, N, J);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_2, Eigen::Dynamic, CELERITE_J2, CELERITE_J_ORDER>> S(S_out, N, J*J);
