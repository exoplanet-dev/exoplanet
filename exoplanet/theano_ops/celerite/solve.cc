    ...
    PyArrayObject** output2)
{
  ...
  auto F_out    = allocate_output<DTYPE_OUTPUT_1>(2, shape1, TYPENUM_OUTPUT_1, output1, &success);
  auto G_out    = allocate_output<DTYPE_OUTPUT_2>(2, shape2, TYPENUM_OUTPUT_2, output2, &success);
  if (success) return 1;
  ...
  Eigen::Map<Eigen::Matrix<DTYPE_INPUT_4, Eigen::Dynamic, CELERITE_NRHS, CELERITE_NRHS_ORDER>> Y(Y_in, N, Nrhs);

  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_0, Eigen::Dynamic, CELERITE_NRHS, CELERITE_NRHS_ORDER>>   Z(Z_out, N, Nrhs);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_1, Eigen::Dynamic, CELERITE_JNRHS, CELERITE_JNRHS_ORDER>> F(F_out, N, J*Nrhs);
  Eigen::Map<Eigen::Matrix<DTYPE_OUTPUT_2, Eigen::Dynamic, CELERITE_JNRHS, CELERITE_JNRHS_ORDER>> G(G_out, N, J*Nrhs);
