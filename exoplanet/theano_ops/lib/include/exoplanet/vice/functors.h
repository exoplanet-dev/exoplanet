...
class LimbDarkFunctor {
 ...
 public:
  ...
  int setup(T r, T x, T xt, T xtt, T y, T yt, T ytt, Scalar z, Scalar zt,
            Scalar dt, std::vector<Scalar>& limits,
    ...
    //         auto limit = (-b0_.value() + sgn2 * (1 + sgn1 * r_.value())) /
    //         v0_.value();
    ...
    //     for (int sgn1 = -1; sgn1 <= 1; sgn1 += 2) {
    //       for (int sgn2 = -1; sgn2 <= 1; sgn2 += 2) {
    //         auto arg = v2 - 4 * a0_.value() * (b0_.value() - sgn2 * (1 + sgn1
    //         * r_.value()));
    //         if (arg < 0) continue;
  ...
  T operator()(Scalar t) {
    ...
    auto S = L_->S;
    auto grad = L_->dSdb.dot(cvec_) * b.derivatives() +
                L_->dSdr.dot(cvec_) * r_.derivatives();
