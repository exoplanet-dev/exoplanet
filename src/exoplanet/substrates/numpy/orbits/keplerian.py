...
class KeplerianOrbit:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if Omega is None:
            ...
            self.Omega = compat.as_tensor(Omega)
        ...
        if ecc is None:
            self.ecc = None
            self.M0 = 0.5 * pi + np.zeros_like(self.n)
            incl_factor = 1
        else:
            self.ecc = compat.as_tensor(ecc)
            if omega is not None:
                ...
                self.cos_omega = compat.as_tensor(cos_omega)
                self.sin_omega = compat.as_tensor(sin_omega)
                self.omega = np.arctan2(self.sin_omega, self.cos_omega)
            ...
            E0 = 2 * np.arctan2(
                np.sqrt(1 - self.ecc) * self.cos_omega,
                np.sqrt(1 + self.ecc) * opsw,
            )
            self.M0 = E0 - self.ecc * np.sin(E0)

            ome2 = 1 - self.ecc ** 2
            self.K0 /= np.sqrt(ome2)
        ...
        if b is not None:
            ...
            self.incl = np.arccos(self.cos_incl)
        elif incl is not None:
            if duration is not None:
                raise ValueError(
                    "only one of 'incl', 'b', and 'duration' can be given"
                )
            self.incl = compat.as_tensor(incl)
            self.cos_incl = np.cos(self.incl)
            ...
            self.duration = compat.as_tensor(to_unit(duration, u.day))
            c = np.sin(pi * self.duration * incl_factor / self.period)
            c2 = c * c
            ...
            zla = np.zeros_like(self.a)
            self.incl = 0.5 * pi + zla
            self.cos_incl = zla
            self.b = zla
        ...
        if t0 is None and t_periastron is None:
            t0 = np.zeros_like(self.period)

        if t0 is None:
            self.t_periastron = compat.as_tensor(t_periastron)
        ...
        self.sin_incl = np.sin(self.incl)
    ...
    def _warp_times(self, t, _pad=True):
        if _pad:
            return compat.as_tensor(t)[..., None] - self.t0
        return t - self.t0
    ...
    def _get_true_anomaly(self, t, _pad=True):
        ...
        return compat.ops.kepler(M, self.ecc + np.zeros_like(M))

    def _get_position_and_velocity(self, t, parallax=None):
        cosf, sinf = self._get_true_anomaly(t)
        ...
        pos = np.concatenate(
            (
                np.sum(self.a_star[..., None] * pos, axis=0, keepdims=True),
                self.a_planet[..., None] * pos,
            ),
            axis=0,
        )
        ...
        vel = np.concatenate(
            (
                np.sum(
                    self.m_planet[..., None] * vel,
                    axis=0,
                    keepdims=True,
                ),
                -self.m_star[..., None] * vel,
            ),
            axis=0,
        )
    ...
    def _get_position(self, a, t, parallax=None, light_delay=False, _pad=True):
        ...
        cosf, sinf = self._get_true_anomaly(t, _pad=_pad)
        if self.ecc is None:
            r = a
        else:
    ...
    def _get_retarded_position(self, a, t, parallax=None, z0=0.0, _pad=True):
        ...
        if self.ecc is None:
            ...
            r = a * (1.0 - self.ecc ** 2) / (1 + self.ecc * cosf)
            vamp = angvel * a / np.sqrt(1 - self.ecc ** 2)
        ...
        if _pad:
            new_t = compat.as_tensor(t)[..., None] - delay
        else:
            new_t = t - delay
    ...
    def get_relative_angles(self, t, parallax=None, light_delay=False):
        ...
        rho = np.squeeze(np.sqrt(X ** 2 + Y ** 2))  # arcsec
        theta = np.squeeze(np.arctan2(Y, X))  # radians between [-pi, pi]
    ...
    def get_planet_velocity(self, t):
        ...
        """
        return tuple(
            np.squeeze(x) for x in self._get_velocity(-self.m_star, t)
        )
    ...
    def get_star_velocity(self, t):
        ...
        """
        return tuple(
            np.squeeze(x) for x in self._get_velocity(self.m_planet, t)
        )
    ...
    def get_relative_velocity(self, t):
        ...
        """
        return tuple(
            np.squeeze(x) for x in self._get_velocity(-self.m_total, t)
        )
...
def get_true_anomaly(M, e, **kwargs):
    ...
    """
    cosf, sinf = compat.ops.kepler(M, e)
...
def get_aor_from_transit_duration(duration, period, b, ror):
    ...
    sinp = np.sin(phi)
    cosp = np.cos(phi)
    num = np.sqrt(opk2 - b2 * cosp ** 2)
    aor = num / sinp
    grad = pi * cosp * (b2 - opk2) / (num * period * sinp ** 2)
    return aor, grad
...
def _get_consistent_inputs(a, period, rho_star, r_star, m_star, m_planet):
    ...
    if m_planet is not None:
        m_planet = compat.as_tensor(to_unit(m_planet, u.M_sun))

    if a is not None:
        a = compat.as_tensor(to_unit(a, u.R_sun))
        if m_planet is None:
            m_planet = np.zeros_like(a)
    if period is not None:
        period = compat.as_tensor(to_unit(period, u.day))
        if m_planet is None:
            m_planet = np.zeros_like(period)
    ...
    if a is not None and period is not None:
        ...
        if r_star is None:
            ...
            r_star = compat.as_tensor(to_unit(r_star, u.R_sun))

        # Compute the implied mass via Kepler's 3rd law
        m_tot = 4 * pi ** 2 * a ** 3 / (G_grav * period ** 2)

        # Compute the implied density
        m_star = m_tot - m_planet
        vol_star = 4 * pi * r_star ** 3 / 3.0
    ...
    if rho_star is not None and not implied_rho_star:
        if has_unit(rho_star):
            rho_star = compat.as_tensor(
                to_unit(rho_star, u.M_sun / u.R_sun ** 3)
            )
        else:
            rho_star = compat.as_tensor(rho_star) / gcc_per_sun
    if r_star is not None:
        r_star = compat.as_tensor(to_unit(r_star, u.R_sun))
    if m_star is not None:
        m_star = compat.as_tensor(to_unit(m_star, u.M_sun))

    # Work out the stellar parameters
    if rho_star is None:
        rho_star = 3 * m_star / (4 * pi * r_star ** 3)
    elif r_star is None:
        r_star = (3 * m_star / (4 * pi * rho_star)) ** (1 / 3)
    elif m_star is None:
        m_star = 4 * pi * r_star ** 3 * rho_star / 3.0

    # Work out the planet parameters
    if a is None:
        a = (G_grav * (m_star + m_planet) * period ** 2 / (4 * pi ** 2)) ** (
            1.0 / 3
        )
    elif period is None:
        period = (
            2 * pi * a ** (3 / 2) / (np.sqrt(G_grav * (m_star + m_planet)))
        )
