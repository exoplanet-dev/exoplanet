...
We could follow \citep{Kipping:2013}.
Alternatively we could sample in $\bvec{q} = (q_1, q_2, q_3)$ with the constraint that $q_1 + q_2 + q3 = 1$
...
This parameterization is included within \exoplanet, but implies a non-uniform marginal for radius ratio (\dfmtodo{make plot}) so we recommend parameterizing as
\begin{eqnarray}
...
where
\begin{proof}{celerite-integral}
  A = 2\frac{C_1\,\left[\cosh(c\,\Delta)\,\cos(d\,\Delta) - 1\right]
  - C_2\,\sinh(c\,\Delta)\,\sin(d\,\Delta)
}{\Delta^2\,(c^2 + d^2)^2}
\end{proof}
and
\begin{proof}{celerite-integral}
  B = 2\frac{C_2\,\left[\cosh(c\,\Delta)\,\cos(d\,\Delta) - 1\right]
