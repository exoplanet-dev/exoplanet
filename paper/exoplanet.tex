...
An important implementation note is that the diagonal of the \celerite\ matrix is not simply $A$ (as we would normally get when using \celerite) because the lag on the diagonal is $\tau = 0$.
This means that the diagonal entries will be
\begin{eqnarray}
A + \frac{2\,\left[
  (a\,c + b\,d)\,(c^2 + d^2)\,\Delta - C_1\,\sinh(c\,\Delta)\,\cos(d\,\Delta) + C_2\,\cosh(c\,\Delta)\,\sin(d\,\Delta)\right]}{[\Delta\,(c^2+d^2)]^2}
\end{eqnarray}
...
The power spectrum of the time-integrated kernel simply
